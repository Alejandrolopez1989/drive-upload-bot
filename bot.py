...
# Asegúrate de que sea un entero. Si no está, se usa 0 por defecto.
try:
...
# Diccionarios en memoria (se reinician al reiniciar el bot)
active_operations = {}
user_credentials = {}
login_states = {}
pending_emails = {}
...
# --- Función actualizada para establecer comandos del bot ---
...
async def list_approved_users_command(client: Client, message: Message):
    ...
    for user_id in approved_users:
        ...
        try:
            # Pyrogram no tiene un método directo para obtener info de un usuario por ID fácilmente sin haber chateado.
            # Si ya tienes una referencia (como message.from_user), puedes usarla.
            # Para obtener info de un usuario arbitrario, necesitas usar get_users, pero requiere permisos y puede no funcionar siempre.
            # Una aproximación es usar el ID directamente y dejar que el admin lo identifique.
            # Si quieres mostrar nombre/username, necesitarías almacenarlos cuando el usuario envía el correo.
            # Como alternativa, puedes mostrar solo el ID y decir que el nombre se puede ver en el mensaje de aprobación.
            
            # Opción 1: Solo mostrar ID (más simple y seguro)
            # response_text += f"- `{user_id}`\n"
            
            # Opción 2: Intentar mostrar información almacenada (requiere modificar el flujo para guardarla)
            # Por ahora, mostramos solo el ID como antes, pero puedes mejorar esto si guardas la info.
            
            # Como no almacenamos nombre/username en este código, solo mostramos el ID.
            # Si quisieras almacenar la info del usuario al enviar el correo, podrías hacerlo en pending_emails o un dict nuevo.
            # Por ejemplo: user_info = {user_id: {'name': ..., 'username': ...}}
            
            # Para este ejemplo, solo mostramos el ID.
            response_text += f"- `{user_id}`\n"
            
        except Exception as e:
            logger.warning(f"No se pudo obtener info del usuario {user_id}: {e}")
            response_text += f"- `{user_id}` (Info no disponible)\n"
...
if __name__ == "__main__":
    async def run_bot():
        ...
        await set_bot_commands(app_telegram) # <-- Esta función se actualizó
